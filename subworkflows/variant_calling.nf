...
workflow VARIANT_CALLING {
    ...
    if (params.caller == 'bcftools') {
        bcftools_out = BCFTOOLS_CALL(ch_markdup_alignment, ch_ref, ch_intervals)
        ...
        if (params.use_bqsr) {
            ...
            recalibrated_alignment = GATK_BQSR(ch_markdup_alignment, ch_bqsr_panel, ch_ref)
            hc_input = recalibrated_alignment.recal_alignment
        }

        hc = GATK_HAPLOTYPECALLER(hc_input, ch_ref, ch_intervals)
        gdb = GATK_GENOMICSDBIMPORT(hc.gvcf, ch_ref)

        if (params.cleanup_GVCFS) {
            GATK_CLEANUP_GVCF(hc.gvcf, gdb.gendb)
        }

        gt = GATK_GENOTYPEGVCFS(gdb.gendb, ch_ref)
        merged_raw = GATK_MERGE_RAW(gt.raw_vcf)
        gatk_filtered = GATK_FILTER(merged_raw.raw_merged, ch_ref)
